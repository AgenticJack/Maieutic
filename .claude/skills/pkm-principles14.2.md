...
**Note output is pure Markdown — no wrapper tags.** A note file contains only its YAML frontmatter and Markdown body. **Never write XML/HTML-style wrapper tags into a note** — in particular `<content>` / `</content>`, `<note>`, `<body>`, or any stray closing tag. These are not Markdown; Obsidian renders them in red as broken syntax. If content was handed to you wrapped in such tags, strip the tags and write only what's inside. The note must end with its last real line (the bottom Synthesis entry), nothing after.
